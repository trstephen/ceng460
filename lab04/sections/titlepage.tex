\begin{titlepage}

\center

% Header
\textsc{\LARGE University of Victoria}\\[1cm] 	% Name of your university/college
\textsc{\Large CENG 460}\\[0.5cm] 			% Major heading such as course name
\textsc{\large Computer Communication Networks}\\[0.5cm] 		% Minor heading such as course title


% Lab Title
\HRule\\[0.4cm]
{\huge \bfseries Lab 4 - TCP}\\[0.2cm] % Title of your document
\HRule\\[1.5cm]


%Lab Instructor Details
\begin{minipage}{0.7\textwidth}
\begin{flushleft}

\large\emph{Instructor:} \\
Dr. Lin \textsc{Cai} \\
\vspace{12 pt}
\emph{Teaching Assistant:} \\
Amir \textsc{Andaliby}

\end{flushleft}
\end{minipage}
~
%% No content here, but it keeps the alignment of the instructor/TA
%% box correct.
%% Consider revising.
\begin{minipage}{0.1\textwidth}
\begin{flushright} \large

\vspace{12 pt}

\end{flushright}
\end{minipage}\\[2cm]


% Lab members
\Large Tyler \textsc{Stephen}
\large V00812021	\\
A01 - B04\\[1.5cm] 


% Date
{\large April 4, 2016}\\ % Date, change the \today to a set date if you want to be precise

% Logo
\begin{figure}[b]	 % put logo at bottom of the page
	\centering
	%\includegraphics[scale=0.3]{../graphics/UVic_logo}
\end{figure}

\end{titlepage}
